...
try {
	...
	do{ 
		...
		pstmt = conn.prepareStatement("select iddd from guest where iddd = ?"); //게스트 테이블에 방금 만든 아이디가 중복되나?
	...
	pstmt = conn.prepareStatement("insert into guest (iddd, sessionid) values (?, ?)");
	pstmt.setString(1, guestid);
	pstmt.setString(2, session.getId()); 
	
	pstmt.executeUpdate(); //sql 실행
	session.setAttribute("iddd", guestid); //저장
	
	response.sendRedirect("gueststart.jsp");
}catch(Exception e){
    out.print(e);
} finally {
